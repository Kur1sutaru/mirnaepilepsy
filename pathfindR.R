...
pathwaysresults<-run_pathfindR(commongeneskegg, 
                               gene_sets = "KEGG",
                               min_gset_size = 3,
                               max_gset_size = 300,
                               custom_genes = NULL,
...
write.csv(pathwaysresults, "kegg_commondegs.csv")
enrichment_chart(pathwaysresults, top_terms = 20)
term_gene_graph(pathwaysresults, num_terms = 3,use_description = TRUE)
term_gene_heatmap(pathwaysresults, degsgse168375, num_terms = 10, use_description = TRUE)
